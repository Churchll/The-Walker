...
class Game:
    def __init__(self,menu):
        ...
        # Append Body
        for i in range(4):
            self.bg.append(image.load("Sprites/Boss/e(0)/e(%d).png" % i))
        for i in range(2):
            self.gui[0].append(image.load("Sprites/Health/hp(%d).png" % i))
        self.target = image.load("Sprites/Main/target.png")
    ...
    def spawnBoss(self):
        ...
        for i in range(6):
            boss = (Boss([],[],[],[],100))
            boss.pos = [1320+i*230,p.pos[Y]]
            boss.vel = [30,10]
            boss.img = self.bg
            boss.frame = [0,0]
        self.bossBattle = True
        self.genBoss = False
        print(self.bossBattle)
    ...
    def run(self):
        ...
        if self.menu == "Main":
            ...
            p.attack()
            if self.bossBattle:
                boss.drawBoss() # Draw the boss
                boss.moveBoss() # Move the boss
...
class gameMenu:
    ...
    def draw(self):
        ...
        self.drawHealth()
        #font.Font.render("Hello", True, (250,250,250))
...
class world:
    ...
    def updateMap(self):
        if p.move and game.bossBattle!= True:
...
class player:
    ...
    def attack(self):
        game.screen.blit(game.target,(game.mPos[X]-50,game.mPos[Y]-50))
        if self.atkAni == False and game.mb[0] == 1:
...
class Boss:
    def __init__(self,pos,vel,img,frame,hp):
        self.pos = pos
        self.vel = vel
        self.frame = frame
        self.hp = hp
...
def main():
    global game,m,cm,gm,w,p,e,b,fx,music,atk
    global X,Y
    global BEE,ROCK
